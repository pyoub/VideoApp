...
export class VideoPlayer extends Component {
  ...
  componentDidMount() {
    ...
    console.log("adddddddddd", request.build);
  }
  constructor(props) {
    super(props); 
    this.state = {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.data.list !== undefined &&
      nextProps.data.list.length > 0 &&
      this.state.Next
    ) {
  ...
  render() {
    return <View style={{ flex: 1 }}>
        ...
        <View style={{ width: "100%", top: 3, height: "20%", justifyContent: "center" }}>
        <Banner unitId={"ca-app-pub-3940256099942544/6300978111"} request={request.build()} size={"LARGE_BANNER"} onAdLoaded={() => {
